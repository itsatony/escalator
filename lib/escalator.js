...
aEscalator.prototype.add = function(id, priority, executor, parameters, scope, resetNeeded, delay) {
	...
	var randomId = getRandomId(this.steps);
	var thisStep = {
		id: randomId
	};
...
/* -----------------------------------------------------------------
 *	method: aEscalator.prototype.set
 *		set a value in the Escalator
 *
 *	parameters: 
 *		key - {String}
 *		value	
 ...
 *
 *	------------------------------------------------------------------*/
aEscalator.prototype.set = function(key, value) {
	this[key] = value;
	return this;
}
/* -----------------------------------------------------------------
...
function getRandomId(steps) {
	var randomId = 'step_' + Date.now() + '_' + Math.floor(Object.keys(steps).length * Math.random() * 10000);
	return typeof steps[randomId] === 'undefined' ? randomId : getRandomId(steps);
}
